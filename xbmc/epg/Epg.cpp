...
bool CEpg::UpdateEntry(const CEpgInfoTag &tag, bool bUpdateDatabase /* = false */, bool bSort /* = true */)
{
  ...
  if (bUpdateDatabase)
    m_changedTags.insert(make_pair<int, CEpgInfoTagPtr>(infoTag->UniqueBroadcastID(),(CEpgInfoTagPtr) infoTag));
...
bool CEpg::FixOverlappingEvents(bool bUpdateDb /* = false */)
{
  ...
  for (map<CDateTime, CEpgInfoTagPtr>::iterator it = m_tags.begin(); it != m_tags.end(); it != m_tags.end() ? it++ : it)
  {
    ...
    if (previousTag->EndAsUTC() >= currentTag->EndAsUTC())
    {
      ...
      if (bUpdateDb)
        m_deletedTags.insert(make_pair<int, CEpgInfoTagPtr>(currentTag->UniqueBroadcastID(),(CEpgInfoTagPtr) currentTag));
    ...
    else if (previousTag->EndAsUTC() > currentTag->StartAsUTC())
    {
      ...
      if (bUpdateDb)
        m_changedTags.insert(make_pair<int, CEpgInfoTagPtr>(currentTag->UniqueBroadcastID(),(CEpgInfoTagPtr) currentTag));
    ...
    else if (previousTag->EndAsUTC() < currentTag->StartAsUTC())
    {
      ...
      {
        m_changedTags.insert(make_pair<int, CEpgInfoTagPtr>(currentTag->UniqueBroadcastID(),(CEpgInfoTagPtr) currentTag));
        m_changedTags.insert(make_pair<int, CEpgInfoTagPtr>(previousTag->UniqueBroadcastID(), (CEpgInfoTagPtr)previousTag));
      }
